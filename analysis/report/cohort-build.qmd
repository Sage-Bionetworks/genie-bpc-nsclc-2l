...
- **Plat chemo before 2L** These people had at least one platinum-containing therapy before their second line therapy. Platinum chemotherapy is defined as carboplatin, cisplatin or oxaliplatin for this analysis.
- **anti PD-1/PD-L1 before 2L** These people had at least one anti-PD-1 or anti-PD-L1 therapy. Any regimen containing pembrolizumab,
  'nivolumab',
  'atezolizumab',
  'durvalumab'
...
```{r}
cohort_cpt_opt <- readr::read_rds(
  here('data', 'drug', 'cohort_with_full_cpt_options.rds')
)

ft_cpt_opt <- cohort_cpt_opt %>%
  summarize(
    `No timing requirement` = n(),
    `Pathology <= index` = sum(any_path_b4_reg),
    `Order <= index` = sum(any_order_b4_reg),
    `Report <= index` = sum(any_report_b4_reg)
  ) %>%
  t %>%
  as_tibble(rownames = "Option", .name_repair = "universal") %>%
  rename(n = 2) %>%
  flextable(.) %>%
  autofit(.)
...
Next-generation sequencing tests (aka cancer panel tests) have three times recorded in GENIE:
  
  - Date of pathology procedure corresponding to the NGS (`path_proc_cpt_rep_days`).
- Date of the order for the NGS test (`cpt_order_int`).
- Date of the report (results) for the NGS test (`dob_cpt_report_days`).

Pathology procedure and report date are always filled, order date is sometimes missing (~5% in this cohort).

Pathology is always on/before the order date, and the order date is always on/before the report date.  It follow that the smallest cohort is obtained (of these three options) when we require that the report is back before the index therapy.  That is what we're currently doing.  The following table shows the cohort size under the other options:
